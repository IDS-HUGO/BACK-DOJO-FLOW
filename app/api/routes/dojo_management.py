...
def get_dojo_owner_profile(current_user: User, db: Session) -> AcademyProfile:
    """
...
def get_my_dojo(
    ...
    return {
        "academy": {
            ...
            "dojo_name": profile.dojo_name if profile else None,
            "owner_name": profile.owner_name if profile else None,
            "contact_email": profile.contact_email if profile else current_user.email,
        ...
        "subscription": {
            ...
            "joined_date": order.created_at if order else None,
        },
        "user": {
...
def get_my_students(
    ...
    if not profile:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin cannot use this endpoint")
...
def get_my_stats(
    ...
    total_students = db.query(Student).filter(Student.academy_id == profile.id).count()
    active_students = db.query(Student).filter(
        Student.academy_id == profile.id,
        Student.status == "active"
    ).count()
    
    total_teachers = db.query(Teacher).filter(Teacher.academy_id == profile.id).count()
    
    total_classes = db.query(Schedule).filter(Schedule.academy_id == profile.id).count()
